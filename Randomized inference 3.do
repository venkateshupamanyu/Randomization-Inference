...
********************************************************************************
	...
	*matrix of simulated results:
	*mat M_exp1 = J(22,10,.)
	mat M_exp2 = J(21,12,.)
	
	*dependent variable:
	global y_key "value_cons23 value_cons20_mo hh_wkly_earn hh_wkly_dys_wrk_casual"
	*also casual wages and non-ag casual wages (long dataset)
...
**********************************************************************************************
*Create exposure variables for each of the 22 states other than AP (with MFI data in 2010)
**********************************************************************************************
...
*generate total GLP by district in Sept. 2010:
gen aux=GLP
replace aux=. if Month!=1 | Year!=2010
bysort district: egen district_GLP = sum(aux)
drop aux

** FIXME - need to make decisions on where to include SKS (only have GLP)
...
*now we generate the states (only those with positive MFI activity in Sept 2010):	
tab state

gen state_name = state
replace state_name="" if state_name=="jammukashmir"
replace state_name="" if state_name=="andhrapradesh"
replace state_name="" if state_name=="goa"
replace state_name="" if state_name=="punjab"
replace state_name="" if state_name=="tripura"
	*for tripura and sikkim, a single exposed MFI (BSFL) operates, so we should not count them twice.
tab state_name

egen state_id = group(state_name)
tab state_id

*now we must generate the exposure variable for each of these states:

forvalues i=1/21 {
* PF_`i': Portfolio loan invested by MFI in state `i'
	gen aux=GLP
	replace aux=0 if Year!=2010
	replace aux=0 if Month==0
	replace aux=0 if state_id!=`i'
	bys Year Month MFI: egen PF_`i' = sum (aux)
	format PF_`i' %20.4f
	drop aux

	replace PF_`i'=0 if PF_`i'>=.

* Frac_`i': investment by MFI in state `i' divided by total investment by Year & Month
	gen Frac_`i' = PF_`i'/Total_PF if Year==2010 & Month==1

* Frac`i'Pre: value of Frac_`i' by MFI level
	bysort MFI: egen Frac`i'Pre= mean(Frac_`i')

*in Muthoot and Satin file, manage for missing values
	replace Frac_`i'=0 if Frac_`i'>=. & MFI=="Muthoot"
	replace Frac_`i'=0 if Frac_`i'>=. & MFI=="Satin"
...
* generated temp variable using FracAPPre (explained before)
	gen aux`i'=(GLP*Frac_`i')
	replace aux`i'=0 if aux`i'>=.
	bysort district: egen temp`i' = sum (aux`i')
	drop aux`i'




* ExpRatio: a number that represents the extent of Exposure of a district to AP state
* ExpRatio is different from ExpDistAPPre (Notice that ExpDistAPPre is before the crisis)
* Higer ExpRatio implies more exposure to Andhra Pradesh
	gen ExpRatio_`i' = (temp`i'/district_GLP)
	replace ExpRatio_`i'=0 if missing(ExpRatio_`i')
	drop temp`i'
...
*now we collapse the exposure variable by district and state:

collapse (mean) ExpRatio_*, by(state district)

*and then we generate other two variables:


	forvalues i=1/21 {
	*the continuous exposure variable:
	gen exposure1_st`i' = log(1+ExpRatio_`i')
	*the binary exposure variable:
	gen exposure2_st`i'=0
	replace exposure2_st`i'=1 if ExpRatio_`i'>0 & ExpRatio_`i'!=.
	}
...
save "$randinf/exposure_otherstates.dta", replace
...
*changing labels to match tables:
	...
	*exposure:
	label var exposure1 "Log(HH Exposure Ratio) $\times$ Post"
...
*now we merge with the other states exposure:
merge m:1 state_name district_name using "$randinf/exposure_otherstates.dta"
drop if _m==2

	*there are 14,175 unmatched HHs:
	tab exposure2 if _m==1
	/*Any Exposed |
		 Lender |
	   $\times$ |
		   Post |      Freq.     Percent        Cum.
	------------+-----------------------------------
			  0 |     14,175      100.00      100.00
	------------+-----------------------------------
		  Total |     14,175      100.00

	
	all of them are unexposed districts (no MFI lending): */
	tab GLP_2010_noSKS_hh if _m==1
	/*
	   2010 GLP |
	without SKS |
	  scaled by |
...
*in this way, I will assign exposure zero for all other states too:
forvalues i=1/21 {
replace exposure1_st`i'=0 if _m==1
replace exposure2_st`i'=0 if _m==1
}
drop _m


*now, we assign exposure (Xpost) zero for rounds 64 and 66:
forvalues i=1/21 {
replace exposure1_st`i'=0 if round==64 | round==66
replace exposure2_st`i'=0 if round==64 | round==66
}

*now we generate state identifies (so we might exclude them):
gen state_code=.
replace state_code=1 if state_name=="assam"
replace state_code=2 if state_name=="bihar"
replace state_code=3 if state_name=="chandigarh"
replace state_code=4 if state_name=="chhattisgarh"
replace state_code=5 if state_name=="delhi"
replace state_code=6 if state_name=="gujarat"
replace state_code=7 if state_name=="haryana"
replace state_code=8 if state_name=="jharkhand"
replace state_code=9 if state_name=="karnataka"
replace state_code=10 if state_name=="kerala"
replace state_code=11 if state_name=="madhyapradesh"
replace state_code=12 if state_name=="maharashtra"
replace state_code=13 if state_name=="meghalaya"
replace state_code=14 if state_name=="orissa"
replace state_code=15 if state_name=="pondicherry"
replace state_code=16 if state_name=="rajasthan"
replace state_code=17 if state_name=="sikkim" | state_name=="tripura"
replace state_code=18 if state_name=="tamilnadu"
replace state_code=19 if state_name=="uttaranchal"
replace state_code=20 if state_name=="uttarpradesh"
replace state_code=21 if state_name=="westbengal"


*now we run all results:
local depvar=1
foreach y in $y_key {

/*standard model, exposure1:
areg `y' exposure1 i.round $controls [pweight=weight], absorb(state_dist) vce(cluster state_dist)
mat b = e(b)
mat M_exp1[1,`depvar']=b[1,1]
*/

*standard model, exposure2:
areg `y' exposure2 i.round $controls_new [pweight=weight], absorb(state_dist) vce(cluster state_dist)
mat b = e(b)
mat M_exp2[1,`depvar']=b[1,1]

forvalues i=1/21 {

/*standard model, exposure1, state `i':
areg `y' exposure1_st`i' i.round $controls [pweight=weight], absorb(state_dist) vce(cluster state_dist)
mat b = e(b)
mat M_exp1[`i',`depvar'+1]=b[1,1]
*/

*standard model, exposure2, state `i':
areg `y' exposure2_st`i' i.round $controls_new if state_code!=`i' [pweight=weight], absorb(state_dist) vce(cluster state_dist)
mat b = e(b)
mat M_exp2[`i',`depvar'+1]=b[1,1]

}


*now we update depvar:
local depvar=`depvar'+2
...
*running the model for the casual daily wage:

/*standard model, exposure1:
areg dly_wage exposure1 i.round $controls [pweight=weight], absorb(state_dist) vce(cluster state_dist)
mat b = e(b)
mat M_exp1[1,`depvar']=b[1,1]
*/

*standard model, exposure2:
areg dly_wage exposure2 i.round i.labor_type_id#i.round $controls_new [pweight=weight], absorb(state_dist) vce(cluster state_dist)
mat b = e(b)
mat M_exp2[1,`depvar']=b[1,1]
...
/*standard model, exposure1, state `i':
areg dly_wage exposure1_st`i' i.round $controls [pweight=weight], absorb(state_dist) vce(cluster state_dist)
mat b = e(b)
mat M_exp1[`i',`depvar'+1]=b[1,1]
*/

*standard model, exposure2, state `i':
areg dly_wage exposure2_st`i' i.round i.labor_type_id#i.round $controls_new if state_code!=`i' [pweight=weight], absorb(state_dist) vce(cluster state_dist)
mat b = e(b)
mat M_exp2[`i',`depvar'+1]=b[1,1]
...
*running the model for the non-ag casual daily wage:

*standard model, exposure2:
areg dly_wage exposure2 i.round i.labor_type_id#i.round $controls_new if labor_type_id==2 | labor_type_id==4  [pweight=weight], absorb(state_dist) vce(cluster state_dist)
mat b = e(b)
mat M_exp2[1,`depvar']=b[1,1]
...
*standard model, exposure2, state `i':
areg dly_wage exposure2_st`i' i.round i.labor_type_id#i.round $controls_new if state_code!=`i' & ( labor_type_id==2 | labor_type_id==4 ) [pweight=weight], absorb(state_dist) vce(cluster state_dist)
mat b = e(b)
mat M_exp2[`i',`depvar'+1]=b[1,1]
...
/*with the continuous exposure measure:
mat li M_exp1
clear
svmat M_exp1
	
	*renaming:
	rename M_exp11 totcons_coef
	rename M_exp12 totcons_dist
	rename M_exp13 durcons_coef
	rename M_exp14 durcons_dist
	rename M_exp15 learnings_coef
	rename M_exp16 learnings_dist
	rename M_exp17 dworked_coef
	rename M_exp18 dworked_dist
	rename M_exp19 dlywage_coef
	rename M_exp110 dlywage_dist
	
	*now we must generate the position of the coefficient:
	foreach v in totcons durcons learnings dworked dlywage {
	egen `v'_coef_aux = mean(`v'_coef)
	gen `v'_leq = 0
	replace `v'_leq = 1 if `v'_coef_aux>=`v'_dist
	egen `v'_pos = sum(`v'_leq)
	drop `v'_coef_aux `v'_leq
	}

	*save the dataset:
	save "$randinf/stateperm_contexp.dta", replace
	sum
*/	
	
*with the binary exposure measure:
mat li M_exp2
clear
svmat M_exp2
	
	*renaming:
	rename M_exp21 totcons_coef
	rename M_exp22 totcons_dist
	rename M_exp23 durcons_coef
	rename M_exp24 durcons_dist
	rename M_exp25 learnings_coef
	rename M_exp26 learnings_dist
	rename M_exp27 dworked_coef
	rename M_exp28 dworked_dist
	rename M_exp29 dlywage_coef
	rename M_exp210 dlywage_dist
	rename M_exp211 nagdlywage_coef
	rename M_exp212 nagdlywage_dist
	
	*now we must generate the position of the coefficient:
	foreach v in totcons durcons learnings dworked dlywage nagdlywage {
	egen `v'_coef_aux = mean(`v'_coef)
	gen `v'_leq = 0
	replace `v'_leq = 1 if `v'_coef_aux>=`v'_dist
	egen `v'_pos = sum(`v'_leq)
	...
	*save the dataset:
	save "$randinf/stateperm_dummyexp.dta", replace
	sum
	
*now we must generate some graphs and tables:
	...
	*graphs:
	...
	graph twoway histogram `v'_dist, discrete xline(`x')
	graph export "$randinf/`v'_stateperm.pdf", as(pdf) replace
	...
	*summary statistics table:
	...
	outtable using "$tables/RI3.tex", mat(S) replace 
